...
class TestProtocolPi30Max(unittest.TestCase):
    ...
    def test_build_result_QPIRI(self):
        ...
        expected = ['ac_input_voltage=230.0V', 'ac_input_current=21.7A', 'ac_output_voltage=230.0V', 'ac_output_frequency=50.0Hz', 'ac_output_current=21.7A', 'ac_output_apparent_power=5000VA', 'ac_output_active_power=4000W', 'battery_voltage=48.0V', 'battery_recharge_voltage=46.0V', 'battery_under_voltage=42.0V', 'battery_bulk_charge_voltage=56.4V', 'battery_float_charge_voltage=54.0V', 'battery_type=AGM', 'max_ac_charging_current=10A', 'max_charging_current=10A', 'input_voltage_range=UPS', 'output_source_priority=Utility Solar Battery', 'charger_source_priority=Utility first', 'max_parallel_units=6', 'machine_type=Off Grid', 'topology=transformerless', 'output_mode=single machine', 'battery_redischarge_voltage=54.0V', 'pv_ok_condition=As long as one unit of inverters has connect PV, parallel system will consider PV OK', 'pv_power_balance=PV input max power will be the sum of the max charged power and loads power']
        simple_formatter = SimpleFormat({"extra_info": False})
        device_info = DeviceInfo(name="name", serial_number="serial_number", model="model", manufacturer="manufacturer")
